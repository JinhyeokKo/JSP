...
public class FileUtil {
	...
	public static ArrayList<String> multipleFile(HttpServletRequest req, String sDirectory) throws ServletException, IOException {
		...
		for(Part part : parts) {
			...
			String partHeader = part.getHeader("content-disposition");
			String[] phArr = partHeader.split("filename=");
			String originamFileName = phArr[1].trim().replace("\"", "");
			if (!originamFileName.isEmpty()) {
				part.write(sDirectory + File.separator + originamFileName);
			}
			listFileName.add(originamFileName);
		}
		return listFileName;
	}
}
